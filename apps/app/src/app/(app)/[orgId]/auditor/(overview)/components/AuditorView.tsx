    ...
    <div className="flex flex-col gap-10">
      ...
      <div className="flex items-center gap-4">
        {logoUrl && (
          ...
          >
            <Image src={logoUrl} alt={gt('{organizationName} logo', { organizationName })} fill className="object-contain" />
      ...
      <Section title={gt('Business Overview')}>
        <div className="space-y-6">
          ...
          />
          <ContentRow title={gt('Mission & Vision')} content={initialContent['Mission & Vision']} />
        </div>
      </Section>

      {/* System Architecture */}
      <Section title={gt('System Architecture')}>
        <ContentRow title={gt('System Description')} content={initialContent['System Description']} />
      </Section>

      {/* Third Party Dependencies */}
      <Section title={gt('Third Party Dependencies')}>
        <div className="grid gap-6 lg:grid-cols-2">
          <ContentRow title={gt('Critical Vendors')} content={initialContent['Critical Vendors']} />
          <ContentRow
...
function ContentRow({ title, content }: { title: string; content?: string }) {
  ...
  return (
    <div className="space-y-1.5">
      <T>
        <h3 className="text-sm font-medium text-foreground"><Var>{title}</Var></h3>
      </T>
      {hasContent ? (
