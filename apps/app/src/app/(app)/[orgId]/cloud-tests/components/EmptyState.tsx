...
import { T, useGT, msg, useMessages, Branch } from 'gt-next';
...
const CLOUD_PROVIDERS = [
  {
    id: 'aws' as const,
    name: msg('Amazon Web Services'),
    shortName: msg('AWS'),
    description: msg('Scan AWS Security Hub for vulnerabilities and compliance issues'),
  ...
  {
    id: 'gcp' as const,
    name: msg('Google Cloud Platform'),
    shortName: msg('GCP'),
    description: msg('Monitor GCP Security Command Center for security findings'),
  ...
  {
    id: 'azure' as const,
    name: msg('Microsoft Azure'),
    shortName: msg('Azure'),
    description: msg('Check Azure Security Center for compliance data'),
  ...
  aws: [
    {
      ...
      helpText: msg('IAM → Users → Security credentials'),
    },
    {
      id: 'secret_access_key',
      label: msg('Secret Access Key'),
      placeholder: msg('Enter your secret access key'),
      helpText: msg('Provided when creating the access key'),
      type: 'password',
    },
  ],
  gcp: [
    {
      id: 'organization_id',
      label: msg('Organization ID'),
      placeholder: '123456789012',
      helpText: msg('Console → IAM & Admin → Settings'),
    },
    {
      id: 'service_account_key',
      label: msg('Service Account Key'),
      placeholder: msg('Paste your JSON key here'),
      helpText: msg('IAM & Admin → Service Accounts → Keys → Add Key'),
      type: 'textarea',
    },
  ],
  azure: [
    {
      ...
      label: msg('Subscription ID'),
      placeholder: '00000000-0000-0000-0000-000000000000',
      helpText: msg('Azure Portal → Subscriptions'),
    },
    {
      id: 'AZURE_TENANT_ID',
      label: msg('Tenant ID'),
      placeholder: '00000000-0000-0000-0000-000000000000',
      helpText: msg('Azure Active Directory → Overview'),
    },
    {
      id: 'AZURE_CLIENT_ID',
      label: msg('Client ID'),
      placeholder: '00000000-0000-0000-0000-000000000000',
      helpText: msg('App registrations → Overview'),
    },
    {
      id: 'AZURE_CLIENT_SECRET',
      label: msg('Client Secret'),
      placeholder: msg('Enter your client secret'),
      helpText: msg('App registrations → Certificates & secrets'),
      type: 'password',
    },
  ],
};
...
export function EmptyState({ onBack, connectedProviders = [], onConnected }: EmptyStateProps) {
  ...
  const validateFields = (): boolean => {
    ...
    fields.forEach((field) => {
      if (!credentials[field.id]?.trim()) {
        newErrors[field.id] = gt('This field is required');
      }
    });
  ...
  const handleValidateAws = async () => {
    if (!credentials.access_key_id || !credentials.secret_access_key) {
      setErrors({
        access_key_id: !credentials.access_key_id ? gt('Required') : '',
        secret_access_key: !credentials.secret_access_key ? gt('Required') : '',
      });
      toast.error(gt('Please enter your AWS credentials'));
      return;
    }
    ...
    try {
      ...
      if (result?.data?.success && result.data.regions) {
        ...
        toast.success(gt('Credentials validated! Now select your region.'));
      } else {
        toast.error(result?.data?.error || gt('Failed to validate credentials'));
      }
    } catch (error) {
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      if (result?.data?.success) {
        ...
        toast.error(result?.data?.error || gt('Failed to connect cloud provider'));
      }
    } catch (error) {
          ...
          <Card className="rounded-xl border-2 shadow-lg">
            <CardHeader className="space-y-4 pb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <CardTitle className="text-xl font-semibold"><T>Select AWS Region</T></CardTitle>
                  <CardDescription className="mt-0.5 text-sm">
                    {gt('Credentials verified {awsAccountId}', { awsAccountId: awsAccountId ? `• Account: ${awsAccountId}` : '' })}
                  </CardDescription>
                </div>
              </div>
            </CardHeader>
              ...
              <div className="space-y-2">
                ...
                >
                  <SelectTrigger className="h-11 rounded-lg transition-colors focus-visible:ring-primary">
                    <SelectValue placeholder={gt('Select your AWS region')} />
                  </SelectTrigger>
                  <SelectContent>
      ...
      <div className="container mx-auto flex min-h-[600px] w-full flex-col items-center justify-center gap-8 p-4 md:p-6 lg:p-8">
        ...
        <div className="mx-auto flex max-w-2xl flex-col items-center gap-6 text-center">
          ...
          <div className="space-y-4">
            <T>
              <Branch
                branch={(!!onBack).toString()}
                true={<h1 className="text-4xl font-bold tracking-tight">Add Another Cloud</h1>}
                false={<h1 className="text-4xl font-bold tracking-tight">Continuous Cloud Scanning</h1>}
              />
            </T>
            ...
            (cloudProvider) => (
              ...
              >
                ...
                <CardHeader className="relative space-y-4 pb-4">
                  ...
                  <CardTitle className="text-lg font-semibold">{m(cloudProvider.shortName)}</CardTitle>
                </CardHeader>
                <CardContent className="relative pb-6">
                  <CardDescription className="text-sm leading-relaxed">
                    {m(cloudProvider.description)}
                  </CardDescription>
                </CardContent>
              </Card>
            ),
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <CardTitle className="text-xl font-semibold">
                      {gt('Connect {shortName}', { shortName: m(provider.shortName) })}
                    </CardTitle>
                    <CardDescription className="mt-0.5 text-sm">
                      <T>Enter your credentials to start scanning</T>
                    </CardDescription>
                  </div>
                </div>
              </div>
            ...
            <CardContent className="space-y-5">
                ...
                <div key={field.id} className="space-y-2">
                  ...
                  {field.type === 'select' && 'options' in field && field.options ? (
                    ...
                    >
                      ...
                      >
                        <SelectValue placeholder={gt('Select a region')} />
                      </SelectTrigger>
                      <SelectContent>
                    ...
                    <textarea
                      id={field.id}
                      placeholder={field.placeholder ? m(field.placeholder) : undefined}
                      value={credentials[field.id] || ''}
              ...
              >
                {isConnecting ? (
                  <>
                    <Loader2 className="mr-2 h-5 w-5 animate-spin" />
                    {selectedProvider === 'aws' ? <T>Validating credentials...</T> : <T>Connecting...</T>}
                  </>
                ) : (
                  <>{selectedProvider === 'aws' ? <T>Continue</T> : gt('Connect {shortName}', { shortName: m(provider.shortName) })}</>
                )}
              </Button>
            </CardContent>
          </Card>
        ...
        <div className="flex flex-col items-center gap-6 text-center">
          ...
          <div className="space-y-2">
            ...
            <p className="text-muted-foreground max-w-lg text-lg">
              {gt('{providerName} is now connected. Your first security scan is running...', { providerName: m(provider.name) })}
            </p>
          </div>
