...
export function DataTableDateFilter<TData>({
  ...
  const label = React.useMemo(() => {
    if (multiple) {
      ...
      const dateText = hasSelectedDates ? formatDateRange(selectedDates) : gt('Select date range');
    ...
    const dateText = hasSelectedDate ? formatDate(selectedDates[0]) : gt('Select date');
  ...
  }, [selectedDates, multiple, formatDateRange, title, gt]);
    ...
    <Popover>
      <PopoverTrigger asChild>
        <Button variant="outline" size="sm" className="border-dashed">
          {hasValue ? (
            <div
              aria-label={gt('Clear {title} filter', { title })}
              onClick={onReset}
